...
import MarkdownContent, { MarkdownDocument } from '@/components/Markdown/MarkdownContent'
import {
	Breadcrumb,
	BreadcrumbItem,
	BreadcrumbLink,
...
async function fetchDatenschutzContent() {
    try {
        ...
        const ordnungResponse = await fetch('https://pad.informatik.sexy/s/Datenschutzordnung/download', {
            next: { revalidate: 3600 } // Revalidate every hour
        });
        
        const hinweiseResponse = await fetch('https://pad.informatik.sexy/s/Datenschutzhinweise/download', {
            next: { revalidate: 3600 } // Revalidate every hour
        });
        
        if (!ordnungResponse.ok) {
            throw new Error(`Failed to fetch Datenschutzordnung content: ${ordnungResponse.status}`);
        }
        
        if (!hinweiseResponse.ok) {
            throw new Error(`Failed to fetch Datenschutzhinweise content: ${hinweiseResponse.status}`);
        }
		...
		<>
            <div className="pt-20">
                ...
                {result.success ? (
                    ...
                    <FetchErrorMessage title="der Datenschutzordnung" error={result.error} />
                )}
            </div>
        </>
	)
